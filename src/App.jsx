...
function App() {
  ...
  return (
    <>
    <header className='underline p-4' id='intro'>
    <h1 className='text-4xl'>Les commandes <span className='uppercase text-cyan-500 '>Termux</span>, dangereuses ?</h1>
    </header>
    <main className='py-4'>
      <p>
      ...
      <fieldset className='border p-2 rounded-lg text-center'>
        <legend>Configuration</legend>
        <form className='flex flex-col pb-2 justify-around items-center uppercase text-xl sm:flex-row gap-4' onSubmit={generate}>
          <label htmlFor="choix">Choix :</label>
          <select name="choix" id='choix' className='bg-inherit text-red-400 border p-4 rounded-lg' onChange={generate}>
          <option value="" disabled selected className='text-xl'>Sélectionnez une charge :</option>
            <option value="1" className='text-xl text-black '>Données Whatsapp</option>
            <option value="2" className='text-xl text-black '>Toutes les images</option>
            <option value="3" className='text-xl text-black '>Toutes les video</option>
            <option value="4" className='text-xl text-black '>Toutes les outils termux</option>
          </select>
          <input className='border m-2 p-2 rounded-lg hover:cursor-pointer text-cyan-400 font-bold' type="submit" value="Recuperer" />
        </form>
        <div className='bg-gray-200 p-1 rounded text-black font-mono sm:text-2xl' id="resultat">
